...
extern const int MAX_ISSUE_NUMBER;
extern const int UNUSED_ISSUE_NUMBER; 
extern const std::string COPYRIGHT;
extern const std::string COPYLEFT;                    
...
public:
    ...
    const std::string& getIssueName() const      { return m_issueName;  }
    int getSeverity() const                      { return m_severity;   }
    bool isThisIssueType(int issueNo) const      { return m_lowerBound <= issueNo && issueNo <= m_upperBound; }
...
public:
    ...
    unsigned int getIssueCount() const       { return m_issues.size();       }
